...
public class EditVideoProcessController implements ControllerInter {
    @Override
...
//                    "    지속적으로 오류가 발생할 경우 관리자에게 문의 바랍니다.");
    ...
    private void dataBaseSave(Map<String,String> parameter) throws SQLException {
        ...
        int videoNumber = Integer.parseInt(parameter.get("videoNumber"));

        // 영상 정보를 DB에 등록
        VideoRepository videoRepository = new VideoRepository();
        videoRepository.update(parameter.get("videoName"),
        ...
        serverImageFileName = (parameter.get("serverImageFileName")).equals("default")?videoImageFileRepository.findByNumber(videoNumber).getServerFilePath():parameter.get(serverImageFileName);
        serverImageFileName = serverImageFileName.equals(null)?videoImageFileRepository.findByNumber(videoNumber).getServerFilePath():serverImageFileName;
        videoImageFileRepository.update(videoNumber, serverImageFileName);
